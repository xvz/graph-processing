...
mpiexec -hostfile ./machines -n ${workers} ../release/toolkits/graph_analytics/sssp --engine ${mode} --format adjgps --graph hdfs://$(hostname):54310/user/ubuntu/input/${inputgraph} --source $src --directed 1 2>&1 | tee -a ./logs/${logfile}
tdone="$(date +%s%N)"
