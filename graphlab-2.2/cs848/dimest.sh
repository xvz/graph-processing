...
mpiexec -hostfile ./machines -n ${workers} ../release/toolkits/graph_analytics/approximate_diameter --format snap --graph hdfs://$(hostname):54310/user/ubuntu/input/${inputgraph} 2>&1 | tee -a ./logs/${logfile}
tdone="$(date +%s%N)"
