...
logname=dimest_${inputgraph}_${workers}_${async}_"$(date +%F-%H-%M-%S)"
...
mpiexec -hostfile ./machines -n ${workers} ../release/toolkits/graph_analytics/approximate_diameter --engine ${mode} --format adjgps --graph hdfs://$(hostname):54310/user/ubuntu/input/${inputgraph} 2>&1 | tee -a ./logs/${logfile}
tdone="$(date +%s%N)"
